import { coordinator, genericPrescriber, genericPatient } from "./sampleData.js"
...
/**
 * Returns a token used for auth for the default patient.
 * 
 * Prerequisite: the default patient must be defined in the db
 * before calling this function
 * @returns token for default patient. Null if the login fails.
 */
export const loginAsDefaultPatient = async () => {
    let res = await fetch(`${SERVER_URL}/user/login`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
